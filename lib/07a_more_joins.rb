...
def title_tracks
  ...
  FROM
    tracks
  JOIN albums ON tracks.album = albums.asin AND tracks.song = albums.title
  SQL
end
...
def best_value
  ...
  FROM (
    ...
    GROUP BY
      albums.asin) AS t
  JOIN albums ON t.asin = albums.asin
  WHERE
    albums.price/t.count < 0.50
  SQL
end
...
def expensive_tastes
  ...
  SELECT
    styles.style,
    SUM(albums.price)/count AS avg_price_track
  FROM (
    SELECT
      styles.style,
      COUNT(tracks.*) count
    ...
    GROUP BY
      styles.style) AS t
  JOIN styles ON styles.style = t.style
