...
def craiglockhart_and_tollcross
  ...
  JOIN
    stops d ON d.id = b.stop_id
  WHERE
   c.name = 'Craiglockhart' AND d.name = 'Tollcross'
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT DISTINCT
   a.num, a.company, c.name, e.num, e.company
  FROM
   routes a
  JOIN
   routes b ON (a.company = b.company AND a.num = b.num)
  JOIN
   stops c ON c.id = a.stop_id
  JOIN
   routes d ON d.stop_id = c.id
  JOIN
   routes e ON (d.company = e.company AND d.num = e.num)
  JOIN
   stops f ON f.id = e.stop_id
  WHERE
   b.stop_id = 53 AND f.name = 'Sighthill'
  SQL
end
